...
import { clearGithubSyncCache, getGithubSyncCache, parseGithubCacheImport, setGithubSyncCache } from "../services/githubSubmissionService";
...
export function Settings() {
  ...
  const exportProgress = () => {
    ...
    const contents = JSON.stringify(createProgressExport(progress, cache.lastSyncAt ? cache : undefined), null, 2);
    ...
    setMessage("Progress and available GitHub sync data exported as JSON.");
  };
  const importProgress = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const githubCache = parseGithubCacheImport(parsed);
      if (githubCache) setGithubSyncCache(githubCache);
      else clearGithubSyncCache();
