...
export function ProgressOverview({ stats, averageSolveTime }: { stats: OverallProgress; averageSolveTime?: string }) {
  const cards = [
  ...
  return (
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <div className="panel p-5 sm:col-span-2 xl:col-span-4"><div className="flex items-baseline justify-between gap-4"><div><p className="font-medium text-slate-100">Your NeetCode 150 progress</p><p className="mt-1 text-sm text-slate-400">{stats.completed} of {stats.total} problems completed{averageSolveTime ? ` · Average completed session: ${averageSolveTime}` : ""}</p></div><span className="shrink-0 text-lg font-semibold text-cyan-300">{stats.percentage}%</span></div><ProgressBar value={stats.percentage} className="mt-4" /></div>
    </section>
  );
}
